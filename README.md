...
![Copilot Studio](https://img.shields.io/badge/Microsoft-Copilot%20Studio-0078D4?logo=microsoft)
![Teams](https://img.shields.io/badge/Deployed%20to-Microsoft%20Teams-6264A7?logo=microsoftteams)
![Entra ID](https://img.shields.io/badge/Auth-Microsoft%20Entra%20ID-0078D4)
![Model](https://img.shields.io/badge/Model-GPT--5.5%20Chat-purple)
![Evaluation](https://img.shields.io/badge/Evaluation-Built--in%20Test%20Sets-green)
...
[▶ Click to watch Demo on Loom](https://www.loom.com/share/9c74b463a2d048f6b5e589e506ebb262)
...
NHS organisations maintain extensive HR and employment policy documentation covering annual leave, sick leave, maternity and paternity entitlements, grievance procedures, whistleblowing, flexible working, and employment conditions.

Finding accurate answers often requires staff to manually search lengthy PDF documents or contact HR departments — slow for staff, and a repetitive workload for HR teams.

This project provides a conversational interface over official NHS policy documents, delivered where NHS staff already work (Microsoft Teams), while ensuring every answer remains grounded in approved source material.
...
- **Escalates sensitive situations** (bullying, harassment, personal disputes) to human support channels — HR, trade unions, Freedom to Speak Up Guardians — via a dedicated escalation topic
- Deployed to **Microsoft Teams** with Microsoft Entra ID authentication
...
User
 │
 ▼
Microsoft Teams  ──  Microsoft Entra ID authentication
 │
 ▼
Copilot Studio Agent
 ├── Instructions (grounding + safety rules)
 ├── Custom Topics (greeting, HR escalation, fallback)
 └── Generative Answers
       │
       ▼
Knowledge Sources (managed retrieval + citations)
       │
       ▼
13 official NHS policy documents (PDF)
```
...
| Model | GPT-5.5 Chat |
| Knowledge Store | NHS policy PDFs (file-based knowledge sources) |
| Authentication | Microsoft Entra ID |
| Delivery Channel | Microsoft Teams |
| Evaluation | Copilot Studio built-in Evaluation (test sets) |
| Platform Foundation | Power Platform environment with Dataverse |
...
I also used Copilot Studio's built-in **Evaluation** feature to run two 10-case test sets against the agent — an out-of-the-box capability that required custom development in the pro-code version.
...
## ⚖️ Pro-Code vs Low-Code: What I Learned
...
| **Retrieval control** | Full — chunk size, hybrid search weights, top-k, custom index schema | Managed — upload files, retrieval is handled for you |
...
**The honest headline:** the low-code version reached roughly 90% of the pro-code version's user-facing quality in roughly 10% of the build time. The remaining 10% — retrieval control, deterministic guardrails, bespoke UX — is exactly what clients pay consultants to know they're giving up.

---

## 🛠️ What It Actually Takes to Stand Up Copilot Studio in a Fresh Tenant

The build itself took a weekend. Getting a **brand-new Microsoft 365 tenant** ready for Copilot Studio took real debugging — operational knowledge that is arguably more valuable than the build itself, because it is exactly what goes wrong at real client sites:

1. **Copilot Studio isn't included in Microsoft 365 Business Basic** — it needs its own trial or licence from the admin center Marketplace
...
3. **Self-service trial signups were blocked** by default tenant policy — resolved via the `MSCommerce` PowerShell module (`AllowSelfServicePurchase`) and enabling email-based subscription signups via Microsoft Graph
4. **Two-tier licensing**: the tenant-level Copilot Studio subscription allows the organisation to run agents, but *publishing* requires a **per-user authoring licence** — a distinction that produces a confusing "no user license" error at the final step
5. **Authentication mode gates channels**: Microsoft (Entra ID) auth restricts the agent to Teams/M365/SharePoint channels; the public demo website channel requires no-auth — and premium features like tenant graph grounding lock the authentication setting entirely until disabled

![Authentication options](screenshots/10-security-authentication-options.png)
...
| 03 | teams-escalation-topic-response | HR escalation topic in action |
...
**Syed Ali Haider** — [LinkedIn](https://www.linkedin.com/in/syed-ali-haider-43777821) · [GitHub](https://github.com/alihaider1993)
